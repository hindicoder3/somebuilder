...
export class WordGraphicBuilder {
  ...
  constructor(secretWord: string) {
    ...
    GlobalFonts.registerFromPath(
      join(__dirname, "..", "fonts", this.FONT_NAME),
      "NS"
    );
  }

  addWordGuesses(wordGuesses: string): void {
    this.wordGuesses.push(wordGuesses);
  }

  buildAsBufferedImage(): Buffer {
    ...
    for (let row = 0; row < 6; row++) {
      for (let col = 0; col < 5; col++) {
        const letterGuess = this.wordGuesses[row]
          ? this.wordGuesses[row][col]
          : null;
      ...
      if (letterGuess) {
        ...
        const textStartY =
          startY + 14 + (this.BOX_SIZE - this.FONT_SIZE) * 2.35;
